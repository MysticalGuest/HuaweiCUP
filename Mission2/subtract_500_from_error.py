...
@site: 
@software: PyCharm
@file: subtract_500_from_error.py
@time: 2021/10/21 19:26
"""
import re
...
def subtract(root_dir, file_name, save_path):
    wb_a = openpyxl.load_workbook(file_name, data_only=True)
    ws_a = wb_a.active
    rows_a = ws_a.rows
    # 全部数据
    li_a = list(rows_a)
    ...
    with open("Tag坐标信息.txt", 'r', encoding='utf-8') as txtData:  # 打开文件夹中的文件内容
        ...
        for l in lines[2:]:
            ...
            if l != '\n':
                ...
                # 处理文件名
                res_wb.save(save_path + str(loc[0]) + ".异常.xlsx")
                i += 1
    print("数据计算完成！")
